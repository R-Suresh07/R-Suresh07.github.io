...
The endpoint scores tell us where AHE helped, but not what changed inside the harness. Looking at the selected harnesses and their trajectories reveals three different stories.

### Lead Optimization: A procedure for state, not authoritative state

Automated search came closest here to the diagnosis we reached manually. The original harness left most of the search state inside Qwen’s trajectory, so Meta-Harness quickly began adding structure around candidate tracking, measurement, and submission.
...
### Interaction Point Discovery: Better receptor reasoning, not new evidence

Here, the automated search took a different path from our manual redesign. Rather than changing the source of evidence available to Qwen, it kept trying to improve how Qwen inferred the hidden interaction points from the receptor itself.
