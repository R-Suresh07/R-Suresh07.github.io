...
**Lead Optimization improved when candidate identity and experimental evidence became authoritative enough to support a much broader multi-candidate search. Interaction Point Discovery improved when the harness stopped asking the model to infer conserved ligand behavior from a single receptor and instead exposed ligand-side features that could be compared for recurrence.** Scaffold Hopping provided the counterexample: once novelty and execution were reliable, the remaining bottleneck was increasingly which chemistry to propose.
