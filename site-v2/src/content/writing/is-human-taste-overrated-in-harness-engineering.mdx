...
import IpdPlausibilityFigure from '../../components/figures/IpdPlausibilityFigure.astro';
import IpdReceptorSideFigure from '../../components/figures/IpdReceptorSideFigure.astro';
import IpdHotspotCoverageFigure from '../../components/figures/IpdHotspotCoverageFigure.astro';
...
import LoTaskContractFigure from '../../components/figures/LoTaskContractFigure.astro';
...
<IpdHotspotCoverageFigure />

There was also a simpler interface problem. The original harness exposed the same generic drug-design tools across tasks, even though IPD ultimately requires only three 3D points in a strict CSV format.<Sidenote number="3" align="paragraph">Each row must contain four comma-separated fields: the x, y, and z coordinates followed by the pharmacophore type. The evaluator reads only the first three valid rows.</Sidenote> The agent still had to choose tools, construct `solution.csv`, format it correctly, and submit the file manually. Those extra steps occasionally caused failures that had nothing to do with the scientific prediction. In `smdd_002_Q92769_0`, for example, the model produced three intended points but separated the values with spaces rather than commas, so the evaluator could not parse the submission.
...
<IpdRecurrenceFigure label="FIG. 8" />
